...
class RingerModeTool @Inject constructor(
    ...
    override val id: String = "RINGER_MODE"
    override val displayName: String = "Silent Mode"
    override val description: String = "Controls ringer mode. Actions: SILENT, VIBRATE, NORMAL, STATUS."
    ...
    override suspend fun execute(request: ToolRequest): ToolResult {
        ...
        val action = request.arguments["action"]?.trim()?.uppercase()
            ?: request.arguments["mode"]?.trim()?.uppercase()
            ?: "STATUS"

        ToolsLogger.d("RingerMode tool: action=$action args=${request.arguments}")

        val result = when (action) {
            "STATUS" -> status(startMs)
            "SILENT" -> setMode(RingerMode.SILENT, startMs)
            "VIBRATE" -> setMode(RingerMode.VIBRATE, startMs)
            "NORMAL" -> setMode(RingerMode.NORMAL, startMs)
        ...
        ToolsLogger.d("RingerMode tool finished: action=$action timeMs=$elapsed payload=$payload")
        return result
    }
    ...
    private fun setMode(mode: RingerMode, startMs: Long): ToolResult {
        val outcome = controller.setMode(mode)
        val elapsed = System.currentTimeMillis() - startMs
